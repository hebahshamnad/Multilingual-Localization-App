...
def lambda_handler(event, context):
    try:
        ...
        # Process uploaded files or fetch from S3
        file_contents = ""
        if uploaded_files:
            file_contents = process_uploaded_files(uploaded_files)
        elif brand_context:
            # Get file contents from brand context (for both new and regenerated translations)
            file_contents = get_brand_context_files(username, brand_context)
...
def get_brand_context_files(username, brand_context_id):
    """Get brand context files from S3 based on brand context ID"""
    try:
        dynamodb = boto3.resource('dynamodb')
        table = dynamodb.Table('BrandContexts')
        
        response = table.get_item(Key={'id': brand_context_id})
        if 'Item' not in response or not response['Item'].get('files'):
            return ""
        
        file_contents = ""
        for file_info in response['Item']['files']:
            try:
                ...
                file_extension = file_name.lower().split('.')[-1] if '.' in file_name else 'txt'
        ...
        print(f"Error getting brand context files: {str(e)}")
        return ""
...
{f"Additional Brand Context: {file_contents}" if file_contents else ""}
